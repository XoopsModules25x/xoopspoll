...
<{foreach item=option from=$poll.options}>
  <tr class='<{cycle values='even,odd'}>'>
    <td class='center'><input type='<{$poll.optionType}>' name='<{$poll.optionName}>' value='<{$option.id}>' /></td>
...
<{    if $poll.votes}>
      <input type='button' value='<{$block.langResults}>' onclick="location='<{$xoops_url}>/modules/<{$block.thisModuleDir}>/pollresults.php?poll_id=<{$poll.id}>'" />
<{    /if}>
    </td>
  </tr>
<{/if}>
