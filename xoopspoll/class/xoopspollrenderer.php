<?php
// $Id: xoopspollrenderer.php 8213 2011-11-07 04:43:08Z beckmi $
//  ------------------------------------------------------------------------ //
...
class XoopsPollRenderer
{
    ...
    function assignForm(&$tpl)
    {
		...
		$tpl->assign('poll', array('question' => $this->poll->getVar("question"), 'pollId' => $this->poll->getVar("poll_id"), 'viewresults' => XOOPS_URL."/modules/xoopspoll/pollresults.php?poll_id=".$this->poll->getVar("poll_id"), 'action' => XOOPS_URL."/modules/xoopspoll/index.php", 'options' => $options, 'description' => $this->poll->getVar("description")));
    }
    ...
    function assignResults(&$tpl)
    {
		...
		if ( !$this->poll->hasExpired() && !XoopsPollLog::hasVoted($this->poll->getVar('poll_id'), xoops_getenv('REMOTE_ADDR'),$uid)) {
